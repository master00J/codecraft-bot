/**
 * Admin API Route: Sync Tier Knowledge to AI Documents
 * POST /api/admin/sync-tier-knowledge
 * 
 * Admin-only endpoint to sync subscription tier information to AI knowledge documents
 * Can sync for a specific guild or all guilds that have AI enabled
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // If specific guild ID provided, sync only for that guild
    if (guildId) {
      const result = await syncTierKnowledgeForGuild(guildId);
      if (!result.success) {
        return NextResponse.json({ error: result.error }, { status: 500 });
      }
      return NextResponse.json({
        success: true,
        message: `Tier knowledge synced successfully for guild ${guildId}`,
      });
    }

    // Otherwise, sync for all guilds with AI enabled
    // Get all guilds that have AI feature enabled
    ...
    // Add guilds from ai_settings
    if (aiSettings) {
      aiSettings.forEach(setting => {
        if (setting.guild_id) guildIds.add(setting.guild_id);
      });
    }

    // Also add the ComCraft server ID
    guildIds.add('1435653730799190058');

    const results = [];
    // Convert Set to Array to avoid TypeScript iteration issues
    const guildIdsArray = Array.from(guildIds);
    for (const gId of guildIdsArray) {
      const result = await syncTierKnowledgeForGuild(gId);
      results.push({ guildId: gId, success: result.success, error: result.error });
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Synced tier knowledge for ${successCount} guild(s)${failCount > 0 ? `, ${failCount} failed` : ''}`,
      results,
    });
  } catch (error: any) {
